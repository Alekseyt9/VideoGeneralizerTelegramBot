...
func (uc *SummarizeVideo) Execute(ctx context.Context, videoURL string) (string, error) {
	...
	title, titleErr := uc.transcripts.FetchTitle(ctx, videoID)
	if titleErr != nil {
		uc.log.Error(ctx, "failed to fetch video title", "error", titleErr, "video_id", videoID)
	}
	...
	if title != "" {
		summary = fmt.Sprintf("**%s**\n\n%s", escapeTelegramMarkdown(title), summary)
	}
